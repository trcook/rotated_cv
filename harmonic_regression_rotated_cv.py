...
def loo_hv_harmonic_regression(k, x, y, y_trend, h=0, v=0):
    ...
    return cv_score, true_cv_score,yhats

def inner_loop(j, exp: HRExpConfig,return_raw = False):
    """
    ...
    y = y_trend + err
    cv, true_cv,yhats = loo_hv_harmonic_regression(exp.n_harmonics, x, y, y_trend, h=0, v=0)
    # record true mse on rotated:

    true_mse = np.mean(np.abs(np.fft.fft(err))**2)
    # (np.linalg.norm(np.fft.fft(err),2)**2).mean()
    if return_raw:
        return cv, true_cv, true_mse,yhats,y
...
out_df.to_json('./harmonic_regression_rotated_cv_out.jsonl', lines=True, orient='records')


# PICKUPHERE -- something is wrong in this -- i think the fft plays some problems with the leave one out cross validation idea because at the end of the day, the fft frequencies exhibit very high values at the tails. 
# The alternative issue may simply be that we are not calculating the cv score correctly (b/c of complex numbers)